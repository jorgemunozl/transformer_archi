...
propagation, how we make that? We are going to use the the Monte Carlo estimator \cite{MetropolisUlam1949}: \begin{equation}
...
A multi layer perceptron (MLP) is a nonlinear mapping from a input patern $\mathbf{x}$ to an ouput vector $\mathbf{y}$ \(\mathcal{F}:\mathbb{R}^{\text{in}}\to \mathbb{R}^{\text{out}}\) \cite{RumelhartHintonWilliams1986}
,is the composition of
  ...
  find a arbitrary number of neurons although is a good practice always choose number which are powers of two and an affine map
