...
the complex unit, \(\hbar\) is the Reduced Planck Constant  approximate to \(1.054571817\dots \times 10^{-34} J\cdot s\) and \(\hat{H}\) is a Hermitian linear operator called the
...
\textbf{Position representation} it takes the form of \cite{Zettili2009}
\[
\mathbf{\hat{p}}=-i\hbar \nabla
\]
...
\] Plugin this form on equation \textbf{number}, you can derive that \cite{Zettili2009}:
\[
T(t)=e^{ -iEt/\hbar }
...
\(4\pi\varepsilon_{0} = 1\),\(\hbar = 1\), and \(m_{e} = 1\) \cite{IUPAC_GoldBook_AtomicUnits}.\\
...
\item
  ...
  \(\mathbb{E}(E_{L})\) and with it and the derivative from equation  we are able to begin the back propagation step.
\end{enumerate}
